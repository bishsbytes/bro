...
- The registry ([metric-registry.ts](../../../packages/domain/src/content/metric-registry.ts)) carries two scored metrics (`mood`, `energy`, 5-point, bounds snapshotted per observation), twelve presence tags, and three imported daily series (`sleep_duration` and `steps` summed, `resting_heart_rate` meaned) with 365-day backfill.
- The resolved-day merge ([resolved-day.ts](../../../packages/logic/src/health/resolved-day.ts) / [resolved-series.ts](../../../packages/logic/src/health/resolved-series.ts)) is the one function that answers "what was this metric's value on this local day"; Trends, Body, goals, and habit completion already call it.
- `observations` is indexed by `(metric_slug, local_day)` and snapshots `scale_min`/`scale_max` per row; the check-in writes tag presence and scored values into the same spine.
- Habits domain math ([packages/logic/src/habits/](../../../packages/logic/src/habits/)) exposes scheduled-day expansion, derived completion, and streaks as pure functions — the raw material for the adherence surface.
...
Pure functions in `packages/logic/src/insight/`, no database handle, no React:
...
| --- | --- |
| Catalogue | `packages/domain/src/content/insight-catalogue.ts` (new) |
| Adapter and engine | Daily-signal adapter and engine in `packages/logic/src/insight/`; store in `apps/app/src/insight/insight-store.ts` |
...
| Export UI | New export screen over `packages/logic/src/export/check-in-export.ts` (serialisation unchanged) |

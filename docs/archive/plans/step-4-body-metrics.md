...
**Complete, 16 August 2026.** All five slices are delivered: migration 004, the conflict-tolerant `unit_preferences` table and latest-per-dimension repository resolution; the signed-off canonical/display unit model with exact conversion, compound parsing, render-only rounding, and locale fallback; measurement registry entries for weight, waist, and body fat; default-off overlays and last-of-day trend aggregation; optional check-in measurement fields with transactional canonical writes; the routed Body overview/detail flow with preferred-unit history, goals, shared progress, and trends; `/settings/units` with unmarked locale defaults, explicit per-dimension choices, and live previews; and export format v3 with unit preferences plus continued v1/v2 parsing. The full offline acceptance flow proves that changing stones to kilograms refreshes Today, Body, detail, and Trends without moving canonical observations or goal targets and without a backend request. Tests, typecheck, lint, and token-parity coverage are green; no native work or prebuild was required.

A post-completion review recorded two implementation decisions the plan had not spelled out. First, the three measurement metrics ship `sensitive: true`: weight, waist, and body fat are body data a user may not want in an export handed to a coach, so the sensitive-exclusion path (per [open decision 10](product-domains-and-data.md#open-decisions)) drops their registry entries, observations, tracked-metric rows, and goals alongside the other sensitive metrics; unit preferences remain in the file, as they describe no logged value. Second, `UnitPreferenceRepository.set` updates the dimension's existing latest row in place rather than inserting a new row per choice — read-side latest-per-dimension resolution is unchanged and the two-offline-devices rule still holds for rows arriving from elsewhere, but the replication step must decide deliberately whether a local `set` may mutate a row authored on another device or should insert instead (a read-side concern today, a merge-semantics one under sync).
...
From a measurement on the Body view: target in the preferred unit, optional date, direction inferred. Target entry uses the same field as every other measurement, so a compound preference gets the same two parts here as on the check-in. Progress is derived from the observation series — the same derivation, the same screen patterns, as wheel goals on `/review`.

### Think in stones

`settings/units`, following the life-areas precedent: separate choices for weight, height, other body measurements, and body fat, previewed with live example values (`78.0 kg`, `5 ft 7 in`, `84.0 cm`). Changing one re-renders its surfaces and rewrites nothing. First run shows the locale default unmarked; an explicit choice persists and replicates when sync arrives.

Choosing a compound unit changes the shape of every entry field for that dimension: `st` and `ft` are typed as two adjacent whole numbers, each labelled with the unit it counts (`12` st, `4` lb), rather than as a sentence to be parsed. The keyboard stays numeric, a blank remainder reads as zero, and a remainder that would carry into a whole major is rejected inline. The text form is still parsed — an import or a paste of `12 st 4 lb` resolves the same way — but it is no longer what the app asks a person to produce.
...
**Delivered hand-off:** Health import (step 5) gets canonical units already decided and enforced for mass, length, and fraction, so the import boundary is a conversion into a known target rather than a second unit decision; the `dimension` concept in the registry ready to type imported metrics; aggregation `"last"` proven for the rollup conversation ([open decision 13](product-domains-and-data.md#open-decisions)); a replicated unit preference contract exported in v3; and a Body view ready to show imported series beside user-entered ones. What step 5 must decide before its first write is the precedence rule when user and tracker disagree on the same day (open decision 12) — both rows are kept, the read side picks — and the platform question (open decision 11). Step 5 is the next native work: HealthKit and Health Connect need a prebuild, batched with Phase 3 per the standing plan.

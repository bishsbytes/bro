...
- [trends-screen.tsx](../../apps/app/src/screens/trends/trends-screen.tsx) already renders the full merged chart list (scored, body, consumption metrics) with a period picker — the Insights tab's chart half exists; the screen's four "destination cards" are the link-farm pattern this plan removes.
- [mind-screen.tsx](../../apps/app/src/screens/mind/mind-screen.tsx) holds the insight feed rendering (empty / not-yet teaser / shown list via `InsightStore.load`) plus a mood/energy-only chart duplicate of Trends and a check-in CTA routing to `/`.

...
**In progress, 16 August 2026.** Slice 1's storage core is implemented: migration 005 and deterministic daily rollups, the independent `bro-local.db` manifest, repositories, startup migration, and two-store deletion. Slice 2 is implemented against the locked policy: registry isolation and intrinsic formatting; canonical mapping; DST-aware wake-day attribution; sum/mean/last rollups; deletion-aware touched-day recomputation; imported precedence with retained provenance; and the 365-day/90-day constants. Native backup exclusion remains with the native/prebuild work. Remaining sign-off gates are the platform library verification on real builds (slice 3 entry) and the connect-screen copy plus store-compliance strings (slice 3). Written to be corrected — the store split and pipeline shape follow the product plan; the field-level specifics are a starting position.

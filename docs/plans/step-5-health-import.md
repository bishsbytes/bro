...
- **`bro-local.db` is excluded from platform backup from day one** by opening it in Expo FileSystem's cache directory on Android and iOS. The product database keeps Expo SQLite's durable default location. Disposable means disposable.
...
#### Slice 3 verification record — 16 August 2026

- `react-native-health-connect` 4.1.3 was verified against its changes API, including initial/continuation tokens, pagination, upsertions, deletions, and expired tokens. The committed Android prebuild includes the Expo plugin and six least-privilege read permissions. An arm64 debug APK built successfully with React Native 0.85.3, the new architecture, compile/target SDK 36, and minimum SDK 26. The first all-architecture attempt exhausted host memory while compiling Reanimated; the resource-bounded arm64 build passed.
- `@kingstinct/react-native-healthkit` 14.0.2 was verified at the TypeScript API/config level for anchored quantity/category queries and deleted samples. Expo config introspection produced the HealthKit entitlement and the read-only `NSHealthShareUsageDescription`; background delivery and write permission are disabled. A real iOS EAS build was not run because `eas whoami` reports that this machine is not logged in, so iOS native compatibility remains a required gate rather than an assumed pass.
- The gateway maps sleep, steps, resting heart rate, weight, and body fat to the shared canonical boundary. The single-flight engine performs a 365-day connect snapshot, incremental token imports, deletion-aware re-rollups, token-expiry replacement, raw pruning, and token advancement only after the durable rollup transaction commits. Replacement snapshots preserve durable days outside the covered backfill range.
- Launch and foreground refresh triggers are mounted after both databases migrate. Connect and explicit refresh are exposed by the engine for Slice 4's settings screen. Web and Jest use the unsupported gateway without loading native modules.
- `bro-local.db` opens in the platform cache directory, which is disposable and excluded from device backup on both native platforms. Product rollups remain in `bro.db`.
- The in-app privacy sentence and iOS usage string are present. The remaining Play Console declaration must list Activity (steps), Sleep, Vitals (resting heart rate), and Body measurements (weight and body fat), with the purpose limited to on-device trends and no backend transfer, advertising use, or sharing. Privacy-policy publication and console submission are manual release tasks.
- Automated engine coverage proves backfill/token/prune behavior, incremental deletion, product-commit rollback discipline, expired-token snapshot replacement, preservation of older durable history, and launch/foreground triggering. Physical-device permission, partial-grant, backfill/resume, deletion, timezone, and backup checks remain scheduled on the Android and iOS dev-client builds.
...
#### Slice 4 implementation record — 16 August 2026

- One shared resolved-series adapter applies the locked imported-wins rule per metric and local day, while retaining both the user observations and tracker rollups on the resolved day for provenance. Trends, Body summaries/details, and weight/body-fat goal progress consume that same adapter.
- Trends automatically adds any health metric with durable imported rows. Imported-only sleep, steps, and resting heart rate require no tracking overlay; imported weight and body fat appear even when their daily-check-in toggle is off. Intrinsic values use their fixed sleep/count/bpm formatting and body values continue through unit preferences.
- Body merges imported weight and body fat into their existing series, keeps manual rows editable, labels the selected source, and exposes resting heart rate as read-only. Goal creation, starting value, current value, and progress all read the resolved series.
- History includes days that contain only health rollups. A day with tracker data shows formatted tracker and manual measurement rows together, labels Apple Health/Health Connect provenance, and marks the row selected for the daily value. Manual-only history retains its pre-import presentation.
- `settings/health` provides platform availability, connect/backfill, partial-grant status by metric, last-imported timestamps, explicit refresh, OS settings, and disconnect. Disconnect copy states that durable imported data remains. Unsupported platforms have no Settings entry; a temporarily unavailable connected platform still permits settings access and local disconnect.
- Isolation regressions keep no-connection Trends, Body, History summaries, and Settings unchanged. Focused store/screen tests and the full app suite cover imported precedence, intrinsic formatting, read-only heart rate, resolved goal progress, provenance, partial grants, connect/refresh/disconnect, and the unsupported-platform path.

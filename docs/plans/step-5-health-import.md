...
**Draft, 16 August 2026. Implementation not started.** Sign-off gates before their slices complete: canonical units and local-day attribution for the imported metrics (slice 2); the v1 metric set, precedence rule, backfill depth, and raw retention window (slice 2); the platform library verification on real builds (slice 3 entry); and the connect-screen copy plus store-compliance strings (slice 3). Written to be corrected — the store split and pipeline shape follow the product plan; the field-level specifics are a starting position.

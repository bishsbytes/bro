...
- **No quantified factors.** `alcohol_units` and the `quantifies` relation ([product plan](product-domains-and-data.md#the-shape-of-the-data)) stay future. V1 negative habits are explicit manual completions — an "alcohol-free day" is a deliberate tap, never inferred from the absence of an `alcohol` factor row, because absence means "didn't log", not "didn't drink".
- **No dual-writes between habits and factors.** Completing the `training` habit does not write a `training` factor observation, and tapping the factor does not complete the habit. One tap, one row. Whether habit adherence joins the correlation pool — and how — is step 7's decision, recorded in the hand-off.

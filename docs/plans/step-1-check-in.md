...
- **Several check-ins per day are allowed.** No unique index on `(metricSlug, localDay)` — two offline devices logging the same day must both survive. The day view shows every entry; the trend aggregates per the registry rule (mean for mood and energy). *Superseded in part by [morning-evening-check-ins.md](morning-evening-check-ins.md): a day now holds two named sittings, and saving into a filled one rewrites it. The absence of the unique index, and the reason for it, are unchanged — the one-per-slot rule is enforced in the store, and duplicate rows arriving from sync all survive with one shown per sitting.*

...
- The registry ([metric-registry.ts](../../packages/domain/src/content/metric-registry.ts)) carries two scored metrics (`mood`, `energy`, 5-point, bounds snapshotted per observation), twelve presence factors, and three imported daily series (`sleep_duration` and `steps` summed, `resting_heart_rate` meaned) with 365-day backfill.

...
**The male-specific half of the catalogue is deliberate.** Beyond the original ten, the registry carries `masturbation`, `porn`, and `morning_erection` under a `sexual` category (which also took `sex` off `social`, where it never belonged), plus `hangover`, `muscle_soreness`, `cold_exposure`, `nicotine`, and `long_hours`. Two reasons they are worth their slugs. First, `morning_erection` is a genuine male biomarker that moves with sleep, alcohol, and stress — it is a near-free proxy for much of what this product is trying to help a user see. Second, the abstinence-tracking audience is large, male, and already keeping this record somewhere; meeting it costs one tag each. All five sexual-category and consumption-adjacent tags are `sensitive`, so the existing export exclusion covers them without new machinery. Labels stay flat and descriptive — the correlation copy has to read the same in either direction, and a judged label makes an honest result sound like a verdict.

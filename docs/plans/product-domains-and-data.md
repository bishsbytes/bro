...
**A factor is a boolean forever; quantity is a separate metric.** Some factors will earn numbers — a challenge to reduce alcohol wants units, not taps. The factor is never promoted in place: `alcohol` rows carry exactly value 1, and reusing the value for a quantity would make every existing row ambiguous between "one unit" and "tapped", retroactively corrupting the series a correlation would read. Instead, a quantified counterpart ships as its own dimensional metric — `alcohol_units`, value in canonical units, null scale bounds, per-day aggregation `sum` — declared in the registry with a `quantifies: 'alcohol'` relation. Where a challenge or preference promotes the factor, the panel swaps the tap for a quantity input and one save writes both rows in one transaction: the presence row keeps every binary read path (today, history, trends, correlation) working unchanged, while the counterpart carries the number the challenge measures. The zero is the prize: `alcohol_units: 0` is an explicit, stored alcohol-free day — a genuine "no" as data rather than the inference above — and the dual-write skips the presence row when the quantity is zero, preserving "no zero-value factor rows". The counterpart's canonical unit (UK units, milligrams of caffeine, training minutes) is a per-metric case of [open decision 5](#open-decisions), taken when that metric ships and before its first write. Nothing structural anticipates any of this: the spine's `REAL` value and nullable bounds already suffice, and the registry is code.
...
5. **Which canonical units,** and kilocalories or kilojoules for energy. Changing this after data exists means rewriting every stored value. This decision recurs per metric: each quantified factor counterpart (alcohol units, caffeine milligrams — see [Check-in](#1-check-in--mood-energy-and-factors)) picks its canonical unit when it ships, before its first write.
...
7. **The shared area vocabulary** — **resolved.** The shipped factor set from step 1 is the input half; the wheel and authored challenge tags share twelve permanent `wheel:*` slugs: career, money, health, partner, family, friends, growth, fun, purpose, fatherhood, faith, and sobriety. The first eight are enabled by default. Labels and the exact catalogue are pinned in the [step 3 plan](step-3-wheel-of-life.md#decisions-locked-for-this-step).

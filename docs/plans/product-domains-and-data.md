...
**The catalogue may grow; the shipped panel may not.** A tag costs nothing on a day it is not tapped, but it costs a glance on every day it is *rendered*, and the fifteen-second budget is spent in glances. So the registry carries `defaultEnabled` per tag: the ten shipped with step 1 are on, and everything authored since starts off and is opted into from check-in settings. This is what lets the catalogue answer "does this app track the thing I care about?" for a wide audience without making the default panel answer it for everyone at once. The overlay does the rest — a tag turned on is a `tracked_metrics` row like any other, and the "genuine no" rule above reads `addedAt`/`removedAt` exactly as it always did.

**The broader catalogue stays balanced without bloating the default panel.** Beyond the original ten, the registry carries `masturbation`, `porn`, and `morning_erection` under a `sexual` category (which also took `sex` off `social`, where it never belonged), plus `hangover`, `muscle_soreness`, `cold_exposure`, `nicotine`, `long_hours`, `meditation`, `anxiety`, `family_time`, and `conflict`. That leaves the five categories within one tag of each other while adding both positive and difficult signals to the previously thin Mind and Social groups. The male-specific tags remain deliberate: `morning_erection` is a genuine male biomarker that moves with sleep, alcohol, and stress — a near-free proxy for much of what this product is trying to help a user see — while the abstinence-tracking audience is large and already keeping this record somewhere. Sexual-category and consumption-adjacent tags are `sensitive`, so the existing export exclusion covers them without new machinery. Labels stay flat and descriptive — the correlation copy has to read the same in either direction, and a judged label makes an honest result sound like a verdict.
...
4. **Does the check-in ship a fixed set of metrics, or a user-chosen one?** A fixed set is simpler for a first release; a chosen one is what keeps the daily loop short as the registry grows, and retrofitting it means migrating everyone's implied selection. The tag panel sharpens this: whatever the answer for scored metrics, tags ship in the first release — correlation needs input signal as much as output, and mood and energy alone starve it.

...
**The metric registry is authored content, not data.** Slug, display name, dimension, scale bounds, aggregation rule, category (which check-in panel group a tag renders in), sensitivity, deprecation status, and whether a metric is user-enterable live in the binary and version with the app — the same argument that keeps prompt text out of the database. What the user does with that catalogue *is* data; see below.
...
Not everything fits — though more fits than it first appears: a multi-select is one boolean observation per selected slug, which is exactly what the [tag panel](#1-check-in--mood-energy-and-tags) is. Free text remains a day note, not a metric.
...
### 1. Check-in — mood, energy, and tags

> **Vocabulary note.** These were called **factors** through steps 1–9 and are called **tags** everywhere from 24 August 2026. The rename is code and copy only — `kind: "factor"` was never persisted, so no stored row changed and no migration was needed. Delivery records below that predate the rename describe the same thing under the old name; anything quoting a `FactorSlug`, `listFactors`, or `factor-coverage.ts` is reading pre-rename history.
...
**Mood is the only required check-in score.** Energy, motivation, productivity, and libido all start enabled, and each can be turned off from check-in settings. Enabled means available, not mandatory: once Mood is selected, the user may save immediately or add any subset of the optional scores. Mood alone proves that a check-in happened, so skipping every optional score still produces a valid entry and completes streak and reminder logic. Existing observations remain attached to their original check-in even if their prompt is later disabled.

**The third element is the tag panel — what happened today.** Alcohol, caffeine, training, travel, stress, sex, late screen: the inputs whose delayed effects are the product's example insights. Structurally there is nothing new here — each tag is a boolean metric in the registry, and tapping a tag writes one observation with value 1 for that slug. Untapped means *unrecorded*, not 0; the rule that makes correlation honest is that a day counts as a genuine "no" for a tag only when a check-in happened on it **and** that tag was active in the user's panel that day — the check-in is the evidence the panel was seen, and `trackedMetrics`' `addedAt`/`removedAt` answers whether the tag was there to tap. A user who never enabled the alcohol tag is not logging alcohol-free days. The registry gains a `category` per tag so the panel renders grouped — lifestyle here, body here — the way Flo groups its tags, and the [catalogue overlay](#catalogue-overlay-snapshot) applies unchanged: users choose which tags show, reorder them, and add their own.

**Tags are how breadth of signal survives the fifteen-second loop.** Correlation needs input signal as much as the mood it explains, and a scored scale is expensive per metric while a tag is nearly free — an untapped tag costs nothing on an ordinary day. This is what lets the check-in carry twenty potential signals without becoming a form, and it is why tags belong in the first release rather than arriving as a later metric drop: mood and energy alone are all output and no input, which starves the product's own engine.

**A tag is a boolean forever; quantity is a separate metric.** Some tags will earn numbers — a challenge to reduce alcohol wants quantities, not taps. The tag is never promoted in place: `alcohol` rows carry exactly value 1, and reusing the value for a quantity would make every existing row ambiguous between a quantity and "tapped", retroactively corrupting the series a correlation would read. Instead, a quantified counterpart ships as its own dimensional metric — `alcohol_intake`, stored in a canonical unit with null scale bounds and per-day aggregation `sum` — declared in the registry with a `quantifies: 'alcohol'` relation. Where a challenge or preference promotes the tag, the panel swaps the tap for a quantity input and one save writes both rows in one transaction: the presence row keeps every binary read path (today, history, trends, correlation) working unchanged, while the counterpart carries the number the challenge measures. The zero is the prize: `alcohol_intake: 0` is an explicit, stored alcohol-free day — a genuine "no" as data rather than the inference above — and the dual-write skips the presence row when the quantity is zero, preserving "no zero-value tag rows". The counterpart's canonical unit (ethanol mass for alcohol, caffeine mass, training time) is a per-metric case of [open decision 5](#open-decisions), taken when that metric ships and before its first write. Nothing structural anticipates any of this: the spine's `REAL` value and nullable bounds already suffice, and the registry is code.

**For now, the whole catalogue is visible in the shipped panel.** A tag costs nothing on a day it is not tapped, but it does cost a glance while it is rendered. The registry still carries `defaultEnabled` per tag so that policy can change without changing the overlay model; currently every tag starts on, giving the catalogue and its categories a fair product trial. Users can shorten the panel from check-in settings. A tag turned off is a `tracked_metrics` row like any other, and the "genuine no" rule above reads `addedAt`/`removedAt` exactly as it always did.

**The broader catalogue stays balanced across categories.** Beyond the original ten, the registry carries `masturbation`, `porn`, and `morning_erection` under a `sexual` category (which also took `sex` off `social`, where it never belonged), plus `hangover`, `muscle_soreness`, `cold_exposure`, `nicotine`, `long_hours`, `meditation`, `anxiety`, `family_time`, and `conflict`. That leaves the five categories within one tag of each other while adding both positive and difficult signals to the previously thin Mind and Social groups. The male-specific tags remain deliberate: `morning_erection` is a genuine male biomarker that moves with sleep, alcohol, and stress — a near-free proxy for much of what this product is trying to help a user see — while the abstinence-tracking audience is large and already keeping this record somewhere. Sexual-category and consumption-adjacent tags are `sensitive`, so the existing export exclusion covers them without new machinery. Labels stay flat and descriptive — the correlation copy has to read the same in either direction, and a judged label makes an honest result sound like a verdict.
...
Open, log mood and energy, tap the tags that apply, done — under fifteen seconds, standing up. Everything else is secondary to this not being tedious, because nothing else works if the daily signal stops arriving. The tag panel keeps its place in the loop only because an untapped tag costs nothing; the moment it demands attention on an ordinary day, it has broken the budget.
...
| --- | --- | --- | --- |
| Check-in observations (mood, energy, tags) | `bro.db` | Yes | User-originated; the core record. |
...
| Tier | Contents |
| --- | --- |
| **No account, free, forever** | Unlimited logging of everything — mood, energy, tags, body metrics, habits, food. The wheel of life and its history. Full history everywhere. Tracker import. Basic per-metric trends. Export. |
...
4. **Does the check-in ship a fixed set of metrics, or a user-chosen one? — resolved.** Mood is fixed. Energy, motivation, productivity, and libido start enabled but can each be disabled; tags follow the same overlay. This keeps the initial signal broad while letting the user shorten the daily loop without migrating or deleting observations.
...
16. **Which food database, and bundled or remote?** **Resolved and shipped, 19 August 2026** — proxied through our API, Open Food Facts first, provider-abstracted so UK and US datasets join server-side. Retention is resolved as none for query text; the endpoint is anonymous and aggregate-only. See [domain 8](#8-consumption-logging--drinks-then-food) and the [step 9 sign-off](step-9-food-logging.md#slice-4-implementation-sign-off--19-august-2026).
17. **Barcode scanning at v1?** **Resolved, 19 August 2026 — yes, later, not in v1 food logging.** Batch the camera dependency, permission, prebuild, and physical acceptance with the outstanding native work; search, recents, custom foods, and recipes are the shipped v1 path.
...
24. **Rewrite the privacy screen once, honestly — current scope complete, 19 August 2026.** The shipped screen now names platform backup, proxied food search with no query retention or identity, and optional sync. Revisit the same screen for the remaining AI disclosure before AI launches.

## Sequencing

Each step adds exactly one hard thing.

1. **Check-in — code-complete, native acceptance pending.** Detailed delivery plan: [Step 1: Check-in](step-1-check-in.md). Delivered on 14 August 2026: the first product migration; `observations`, `dayNotes`, and tracked-metric repositories; authored metric registry and tag panel; Today, History/day editing, and free 7/30-day Trends; transactional **delete local data** with its reserved copy; seeded identity-continuity assertions; and a tested version 1 **export serialisation** without share UI. Automated tests, typecheck, lint, and migration regeneration are green. The physical-device airplane-mode, day-boundary, colour-scheme, and fifteen-second acceptance checklist remains before this step is fully complete.

...
**Mood is the only fixed check-in score.** Energy, motivation, productivity, and libido all start enabled, and each can be turned off from check-in settings. Mood alone proves that a check-in happened, so turning Energy off produces a valid Mood-only entry rather than suppressing completion, streaks, or reminder cancellation. Existing observations remain attached to their original check-in even if their prompt is later disabled.

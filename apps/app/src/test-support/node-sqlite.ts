 ...
 *
 * `expo-sqlite/kv-store` reaches its native module through a package-relative
 * import, so mocking `expo-sqlite` alone would not reach it. The store is
 * therefore reimplemented here over the same `storage(key, value)` table it
 * uses, rather than stubbed out.
 */
			...
			prepareAsync: async (sql: string) => {
				...
				return {
					executeForRawResultAsync: async (params: unknown[] = []) => ({
						getAllAsync: async () => statement.all(...(params as never[])),
					}),

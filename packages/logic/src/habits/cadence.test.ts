import { isoWeekdayForLocalDay, shiftLocalDay } from "@bro/domain";
import { localDayAt } from "../health/mapping";
import { isHabitScheduled, scheduledDaysBetween } from "./cadence";
...
describe("habit cadence", () => {
	it("expands Monday-first masks across month and DST boundaries", () => {
		...
		// The expansion walks day by day, so it inherits shiftLocalDay's promise
		// that a spring-forward night neither skips nor repeats a date.
	...
	it("rejects empty masks and backwards ranges", () => {
	...
	it("rejects a local day that is not a real date", () => {
		expect(() => scheduledDaysBetween("2026-02-30", "2026-03-02", 0b1)).toThrow(
			"real YYYY-MM-DD",
		);
	});
});

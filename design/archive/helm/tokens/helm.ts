...
export function deriveAccent(hue: number, dark: boolean) {
	...
	return dark
		? {
				accent: oklchToHex(0.74, ACCENT_CHROMA, h),
				accentDeep: oklchToHex(0.36, ACCENT_CHROMA * 0.7, h),
				onAccent: oklchToHex(0.16, 0.04, h),
			}
		: {
				accent: oklchToHex(0.5, ACCENT_CHROMA, h),
				accentDeep: oklchToHex(0.9, ACCENT_CHROMA * 0.45, h),
				onAccent: oklchToHex(0.98, 0.02, h),
			};
}
...
export function createTheme(
	scheme: "dark" | "light",
	accentHue: number = ACCENT_DEFAULT_HUE,
) {
	...
	return {
		...
		isDark: dark,
		colors: {
			...(dark ? paletteDark : paletteLight),
			...deriveAccent(accentHue, dark),
		},
